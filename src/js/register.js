...
// 账号密码
$('[name="pwd1"]').focus(()=>{
    $('[name="spanPwd1"]').html("请输入长度为8-14个字符,不允许有空格、中文");
    $('[name="spanPwd1"]').css({color:'blue'});
})
// 再次输入密码
$('[name="pwd2"]').focus(()=>{
    $('[name="spanPwd2"]').html("请输入长度为8-14个字符,不允许有空格、中文");
...
$('[name="name"]').on('input',()=>{
    ...
    // console.log(val)
    if(val<3){
        $('[name="name1"]').html(`您已输入了${val}位,还需要输入${3-val}位`)
        $('[name="name1"]').css({color:'red'});

    }
    else if(val ===3){
        $('[name="name1"]').html('您已输入的位数已到达3位');
        $('[name="name1"]').css({color:'green'});

    }
    else if(val>3){
        $('[name="name1"]').html(`您最多输入14位,还能输入${14-val}位`)
        $('[name="name1"]').css({color:'green'});
    }
})
...
$('[name="pwd1"]').on('input',()=>{
    ...
    // console.log(val)
    if(val<8){
        $('[name="spanPwd1"]').html(`您已输入了${val}位,还需要输入${8-val}位`)
        $('[name="spanPwd1"]').css({color:'red'});

    }
    else if(val>=8){
        $('[name="spanPwd1"]').html(`您最多输入14位,还能输入${14-val}位`)
...
$('[name="pwd2"]').on('input',()=>{
    ...
    // console.log(val)
    if(val<8){
        $('[name="spanPwd2"]').html(`您已输入了${val}位,还需要输入${8-val}位`)
        $('[name="spanPwd2"]').css({color:'red'});

    }
    else if(val>=8){
        $('[name="spanPwd2"]').html(`您最多输入14位,还能输入${14-val}位`)
